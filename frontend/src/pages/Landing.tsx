...
export default function LandingPage() {
  ...
  function start(initialMessage?: string) {
    navigate("/chat", { state: { query: initialMessage ?? query.trim() } });
  }
          ...
          <div className="cp-welcome-left">
            ...
            <div className="cp-welcome-popular">
              ...
              <div className="cp-welcome-popular-pills">
                {popular.map((p, i) => (
                  <button key={i} className={`cp-pill${p.v ? ` cp-pill--${p.v}` : ""}`} onClick={() => start(p.l)}>
                    {p.l}
                  </button>
                ))}
              </div>
