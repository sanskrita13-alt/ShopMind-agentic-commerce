...
public class MockAiService implements AiService {
    ...
    public ExtractedIntent extractIntent(List<Map<String, String>> conversationHistory) {
        ...
        // ── Use case ──────────────────────────────────────────────────────────
        if (allMessages.contains("college") || allMessages.contains("campus") || allMessages.contains("university") || allMessages.contains("school")) {
            builder.primaryUseCase("college daily wear");
            missing.remove("primaryUseCase");
        } else if (allMessages.contains("gym") || allMessages.contains("workout") || allMessages.contains("training") || allMessages.contains("exercise") || allMessages.contains("lift")) {
            builder.primaryUseCase("gym training");
            missing.remove("primaryUseCase");
        } else if (allMessages.contains("run") || allMessages.contains("jog") || allMessages.contains("marathon")) {
            builder.primaryUseCase("running");
            missing.remove("primaryUseCase");
        } else if (allMessages.contains("trek") || allMessages.contains("hike") || allMessages.contains("hiking")) {
            builder.primaryUseCase("trekking");
            missing.remove("primaryUseCase");
        } else if (allMessages.contains("office") || allMessages.contains("work") || allMessages.contains("formal")) {
            builder.primaryUseCase("office wear");
            missing.remove("primaryUseCase");
        ...
        if (allMessages.contains("all day") || allMessages.contains("whole day") || allMessages.contains("entire day")
                || allMessages.contains("most of the day") || allMessages.contains("8") || allMessages.contains("9")
                || allMessages.contains("10 hour") || allMessages.contains("long day")) {
            builder.walkingDuration("6-8 hours");
            missing.remove("walkingDuration");
        } else if (allMessages.contains("6") || allMessages.contains("7")
                || allMessages.contains("half day") || allMessages.contains("quite a lot") || allMessages.contains("quite a bit")) {
            builder.walkingDuration("6-8 hours");
            missing.remove("walkingDuration");
        } else if (allMessages.contains("3") || allMessages.contains("4") || allMessages.contains("5")
                || allMessages.contains("few hour") || allMessages.contains("couple of hour") || allMessages.contains("some hour")
                || allMessages.contains("moderate") || allMessages.contains("few hours")) {
            builder.walkingDuration("3-5 hours");
            missing.remove("walkingDuration");
        } else if (allMessages.contains("1") || allMessages.contains("2")
                || allMessages.contains("not much") || allMessages.contains("not a lot") || allMessages.contains("short")
                || allMessages.contains("minimal") || allMessages.contains("occasionally") || allMessages.contains("rarely")) {
            builder.walkingDuration("1-2 hours");
            missing.remove("walkingDuration");
        }

        // ── Budget — handles INR (500–100,000), "k" notation, commas ─────────
        String budgetMsg = allMessages.replaceAll(",", ""); // strip commas: "10,000" → "10000"
        // "Xk" → X * 1000
        java.util.regex.Matcher kMatcher = java.util.regex.Pattern.compile("(\\d+)\\s*k\\b").matcher(budgetMsg);
        if (kMatcher.find()) {
            try {
                builder.budget(Double.parseDouble(kMatcher.group(1)) * 1000);
                missing.remove("budget");
            } catch (NumberFormatException ignored) { }
        } else {
            // plain 3–6 digit number in the INR range 500–100000
            java.util.regex.Matcher numMatcher = java.util.regex.Pattern.compile("\\b(\\d{3,6})\\b").matcher(budgetMsg);
            while (numMatcher.find()) {
                try {
                    double val = Double.parseDouble(numMatcher.group(1));
                    if (val >= 500 && val <= 100_000) {
                        builder.budget(val);
                        missing.remove("budget");
                        break;
                    }
        ...
        // Natural language budget clues (fallback)
        if (missing.contains("budget")) {
            if (allMessages.contains("cheap") || allMessages.contains("affordable") || allMessages.contains("low budget") || allMessages.contains("tight budget")) {
                builder.budget(5000.0);
                missing.remove("budget");
            } else if (allMessages.contains("no budget") || allMessages.contains("don't care about price") || allMessages.contains("any price") || allMessages.contains("not a concern")) {
                builder.budget(30000.0);
                missing.remove("budget");
            }
        }

        // ── Priorities — natural language aware ───────────────────────────────
        boolean styleWords   = allMessages.contains("style") || allMessages.contains("look") || allMessages.contains("appearance")
                || allMessages.contains("fashion") || allMessages.contains("aesthetic") || allMessages.contains("cool")
                || allMessages.contains("want to look") || allMessages.contains("looks good") || allMessages.contains("look good");
        boolean comfortWords = allMessages.contains("comfort") || allMessages.contains("cushion") || allMessages.contains("soft")
                || allMessages.contains("support") || allMessages.contains("ergonomic") || allMessages.contains("feel good")
                || allMessages.contains("feel comfortable") || allMessages.contains("pain") || allMessages.contains("feet hurt");

        if (comfortWords && !styleWords) {
            builder.comfortPriority(0.9);
            builder.stylePriority(0.4);
            missing.remove("comfortPriority");
            missing.remove("stylePriority");
        } else if (styleWords && !comfortWords) {
            builder.stylePriority(0.9);
            builder.comfortPriority(0.5);
            missing.remove("comfortPriority");
            missing.remove("stylePriority");
        } else if (comfortWords && styleWords) {
            builder.comfortPriority(0.7);
            builder.stylePriority(0.7);
            missing.remove("comfortPriority");
            missing.remove("stylePriority");
        } else if (allMessages.contains("both") || allMessages.contains("equal") || allMessages.contains("balance")) {
            builder.comfortPriority(0.7);
            builder.stylePriority(0.7);
            missing.remove("comfortPriority");
            missing.remove("stylePriority");
        } else if (allMessages.contains("doesn't matter") || allMessages.contains("don't care") || allMessages.contains("either")) {
            builder.comfortPriority(0.6);
            builder.stylePriority(0.6);
            missing.remove("comfortPriority");
            missing.remove("stylePriority");
        }

        if (allMessages.contains("durable") || allMessages.contains("last long") || allMessages.contains("long lasting") || allMessages.contains("tough")) {
            builder.durabilityPriority(0.8);
        }

        // ── Fit ───────────────────────────────────────────────────────────────
        if (allMessages.contains("wide") || allMessages.contains("broad")) {
            builder.preferredFit("wide");
            missing.remove("preferredFit");
        } else if (allMessages.contains("narrow") || allMessages.contains("slim fit") || allMessages.contains("tight fit")) {
            builder.preferredFit("narrow");
            missing.remove("preferredFit");
        } else if (allMessages.contains("regular") || allMessages.contains("normal") || allMessages.contains("standard")
                || allMessages.contains("no preference") || allMessages.contains("doesn't matter") || allMessages.contains("don't mind")) {
            builder.preferredFit("standard");
            missing.remove("preferredFit");
        }

        // ── Versatility ───────────────────────────────────────────────────────
        if (allMessages.contains("versatile") || allMessages.contains("both") || allMessages.contains("multiple")
                || allMessages.contains("all purpose") || allMessages.contains("everything")) {
            builder.needsVersatility(true);
        }

        // ── Terrain ───────────────────────────────────────────────────────────
        if (allMessages.contains("trail") || allMessages.contains("outdoor") || allMessages.contains("off-road") || allMessages.contains("mud") || allMessages.contains("hike")) {
